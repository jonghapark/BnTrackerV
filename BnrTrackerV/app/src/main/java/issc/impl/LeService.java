...
import issc.gatt.Gatt.Listener;
import issc.gatt.GattAdapter;
import issc.gatt.GattCharacteristic;
import issc.gatt.GattDescriptor;
import issc.gatt.GattService;
import issc.impl.aosp.AospGattAdapter;
import issc.impl.test.FakeGattAdapter;
...
public class LeService extends Service {
    ...
    class TheCallback implements Gatt.Listener {
        ...
        public void onCharacteristicRead(Gatt gatt, GattCharacteristic chrc, int status) {
            CommonUtil.myLog("------onCharacteristicRead");
            synchronized(mListeners) {
                Iterator<Listener> it = mListeners.iterator();
                while(it.hasNext()) {
        ...
        public void onConnectionStateChange(Gatt gatt, int status, int newState) {
            CommonUtil.myLog("------onConnectionStateChange");
            synchronized(mListeners) {
                Iterator<Listener> it = mListeners.iterator();
                while(it.hasNext()) {
        ...
        public void onDescriptorRead(Gatt gatt, GattDescriptor descriptor, int status) {
            CommonUtil.myLog("------onDescriptorRead");
            synchronized(mListeners) {
                Iterator<Listener> it = mListeners.iterator();
                while(it.hasNext()) {
